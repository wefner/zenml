...
from typing import Any, Dict, Iterator, ClassVar
...
class BaseLogCollector(StackComponent, ABC):
    ...
    def get_logging_level() -> LoggingLevels:
        ...
        if verbosity not in LoggingLevels.__members__:
            raise KeyError(
                ...
                f"{list(LoggingLevels.__members__.keys())}"
            )
        return LoggingLevels[verbosity]

    def set_root_verbosity(self) -> None:
        ...
        if level != LoggingLevels.NOTSET:
            ...
            self.get_logger(self.name).debug(
                f"Logging set to level: " f"{logging.getLevelName(level.value)}"
            )
        else:
            logging.disable(sys.maxsize)
            logging.getLogger().disabled = True
            self.get_logger(self.name).debug("Logging NOTSET")
    ...
    def get_logger(self, name: str) -> logging.Logger:
        ...
        logger.setLevel(self.get_logging_level().value)
    ...
    def init_logging(self) -> None:
        ...
        self.set_root_verbosity()

        # Enable logs if environment variable SUPRESS_ZENML_LOGS is not set 
        # to True
    ...
    def disable_logging(log_level: int) -> Iterator[None]:
        """Contextmanager that temporarily disables logs below a threshold 
        level.
        ...
        Args:
            log_level: All logs below this level will be disabled for the 
            duration of this contextmanager.
        """
